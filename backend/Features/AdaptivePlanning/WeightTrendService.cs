...
namespace backend.Features.AdaptivePlanning
{
    public class WeightTrendService
    {
        ...
        {
            var start = week.Start.AddDays(-14).ToDateTime(TimeOnly.MinValue, DateTimeKind.Utc);
            ...
            var expectedWeeklyChange = GetExpectedWeeklyChange(settings, recentAvg);
            ...
            return new WeightTrendAnalysis
            {
                ...
                Status = status,
                Insight = BuildInsight(status, weeklyChange, expectedWeeklyChange, confidence)
            };
        }
        ...
        private static double? GetExpectedWeeklyChange(UserSettings settings, double? trendWeightKg)
        {
            if (!trendWeightKg.HasValue) return null;
            if (settings.WeightDirection == WeightDirection.Maintain) return 0;

            var today = DateOnly.FromDateTime(DateTime.UtcNow);
            var targetDate = DateOnly.FromDateTime(settings.WeightGoalTimeUtc);
            var daysRemaining = Math.Max(1, targetDate.DayNumber - today.DayNumber);
            var weeksRemaining = daysRemaining / 7.0;
            return ((double)settings.WeightGoalKg - trendWeightKg.Value) / weeksRemaining;
        }
            ...
            DataQualityLevel confidence)
        {
            ...
            {
                "onTrack" => $"Vekttrenden er nær målet ditt ({trend} mot plan {expected}).",
                "slightlyBehind" => $"Vekttrenden er litt bak plan ({trend} mot plan {expected}).",
                "behind" => $"Vekttrenden går tregere enn målet tilsier ({trend} mot plan {expected}).",
                "tooAggressive" => $"Vekttrenden går raskere enn planlagt ({trend} mot plan {expected}).",
                "maintaining" => "Vekttrenden er stabil denne uken.",
                "gaining" => $"Vekttrenden peker oppover ({trend}).",
                "losing" => $"Vekttrenden peker nedover ({trend}).",
